...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f2f5", fontFamily: "'Segoe UI', Arial, sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ background: "#003087", padding: "16px 24px", boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ fontSize: 28 }}>🏛️</div>
            <div>
              <div style={{ color: "white", fontWeight: 700, fontSize: 18 }}>NYS Unemployment Insurance Claims</div>
              <div style={{ color: "#7EC8E3", fontSize: 12 }}>New York State Department of Labor — AI-Powered Claims Filing</div>
            </div>
          </div>
      ...
      {/* Info bar */}
      <div style={{ background: "#001f5b", padding: "10px 24px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", display: "flex", gap: 24, justifyContent: "center" }}>
          {[["💬 < 2 min", "To file a claim"], ["🤖 Claude AI", "Powered chatbot"], ["🗄️ IBM DB2", "Live database"], ["🔒 Secure", "Encrypted session"]].map(([val, label]) => (
            <div key={label} style={{ textAlign: "center" }}>
              ...
              <div style={{ color: "#7EC8E3", fontSize: 11 }}>{label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "24px 16px", minHeight: "calc(100vh - 280px)" }}>
        ...
        {!started ? (
          <div style={{ textAlign: "center", paddingTop: 60 }}>
            ...
            <h2 style={{ color: "#003087", fontSize: 24, marginBottom: 12 }}>NYS Unemployment Insurance Claims</h2>
            <p style={{ color: "#555", fontSize: 15, marginBottom: 8, maxWidth: 500, margin: "0 auto 8px" }}>
            ...
            <p style={{ color: "#888", fontSize: 13, marginBottom: 32 }}>
              Powered by Claude AI · Connected to live IBM DB2 database
            </p>
            <button onClick={startSession} style={{
              background: "#003087", color: "white", border: "none",
              padding: "16px 48px", borderRadius: 8, fontSize: 16,
              fontWeight: 600, cursor: "pointer", boxShadow: "0 4px 12px rgba(0,48,135,0.3)"
            }}>
              Start New Claim
            </button>
            <p style={{ color: "#888", fontSize: 12, marginTop: 16 }}>
              You will need: SSN, Date of Birth, Employer Information, Separation Reason
            </p>
          </div>
        ) : (
          <>
        ...
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "white", borderTop: "1px solid #e8ecf0", padding: "16px 24px", boxShadow: "0 -2px 8px rgba(0,0,0,0.08)" }}>
          <div style={{ maxWidth: 800, margin: "0 auto", display: "flex", gap: 12 }}>
            ...
            <button onClick={send} disabled={loading || !input.trim() || !sessionId}
              style={{ background: loading || !input.trim() ? "#ccc" : "#003087", color: "white", border: "none", borderRadius: 24, padding: "12px 24px", fontSize: 14, fontWeight: 600, cursor: loading || !input.trim() ? "not-allowed" : "pointer" }}>
              Send
            </button>
          </div>
